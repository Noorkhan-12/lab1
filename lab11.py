...
def insert_or_update_user(username, phone):
    conn = sqlite3.connect("game.db")
    c = conn.cursor()
    c.execute("SELECT * FROM PhoneBook WHERE username = ?", (username,))
    if c.fetchone():
        c.execute("UPDATE PhoneBook SET phone = ? WHERE username = ?", (phone, username))
    else:
        c.execute("INSERT INTO PhoneBook (username, phone) VALUES (?, ?)", (username, phone))
    conn.commit()
    conn.close()
...
def insert_many_users(user_list):
    ...
    for username, phone in user_list:
        if is_valid_phone(phone):
            ...
            invalid_entries.append((username, phone))
    conn.commit()
    conn.close()
...
def delete_user(identifier):
    conn = sqlite3.connect("game.db")
    c = conn.cursor()
    c.execute("DELETE FROM PhoneBook WHERE username = ? OR phone = ?", (identifier, identifier))
    conn.commit()
    conn.close()
    ...
    while True:
        ...
        if choice == '1':
            ...
            if op == '1':
                ...
                for r in search_by_pattern(pat): print(r)
            elif op == '8':
                u = input("Enter username: ")
                p = input("Enter phone: ")
                insert_or_update_user(u, p)
            elif op == '9':
                ...
                print("Invalid entries:", bad)
            elif op == '10':
                lim = int(input("Limit: "))
                off = int(input("Offset: "))
                for r in get_paginated_users(lim, off): print(r)
        elif choice == '2':
